...
knitr::opts_chunk$set(echo = FALSE, cache = FALSE, progress = FALSE, 
...
### Project goal 
create a web page presentation using R Markdown that features a plot created with Plotly, and to host the resulting web page on RPubs.

### Result
The interactive plot on the next slide represents the number of road accidents in Great Britain from 2005 to 2016, grouped by severity (slight, serious, or fatal).
...
# - Road Safety - Accidents 2016, from
# http://data.dft.gov.uk/road-accidents-safety-data/dftRoadSafety_Accidents_2016.zip

# Then extract the `Accidents0514.csv` , `Accidents_2015.csv` and 'Accidents_2016.cvs' files from
# the zip files in a subdirectory named `data`.
```


```{r,}
# read data for 2005-2014 , 2015 and 2016 as data tables and keep only severity and
# date columns
...
accidents16 <- fread("data/Accidents_2016.csv", header = TRUE, 
                     sep = ",")
accidents16 <- accidents16 %>%
  select(Accident_Severity, Date)

# concatenate data tables and free up environment
accidents <- rbind(accidents0514, accidents15, accidents16)
rm(list = c("accidents0514", "accidents15", "accidents16"))
...
## Road accidents in GB(2005-2016)
...
### Data source and licence
   Released by the United Kingdom's Department from Transport under the Open Government Licence
